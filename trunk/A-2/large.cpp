...
Description: (A * means it is in the process of being implemented)
	...
	The program will begin by displaying several choices. 
		...
		The fifth choice will be to reset the scores.
		The sixth choice will be to display the scores.
		There is also a hidden choice that is for debugging.
...
Includes:
	...
	string	= PC's don't play nice with strings
 */
#include <iostream>
#include <cstdlib>
#include <ctime>
...
/*
Constants:
	RANDOM_MIN 	= The default random minimum.
	RANDOM_MAX 	= The default random maximum.
...
static const string CHOICES[NUM_CHOICES][2] =
{
...
/*
Functions:
	randomNumber	= Randomly generates between rMin and rMax.
	smartGuess		= Guesses the middle number between the rMin and rMax.
	randomNumber	= Randomly generates a number between two given numbers.
	antiCheat		= Adds to the playes guess count in the case they try and cheat the system.
	reset				= Resets the values.
	updateScores	= Updates the scores
	displayScores	= Displays the scores.
	player			= Runs the game with the player picking a number and the computer trying to guess it.
	computer			= Runs the game with the computer picking a number and the player trying to guess it.
	randomChoose	= Randomly picks either the player or computer to choose a number.
	fullGame			= Runs a full game in which the first player to pick a number is random, and then it switches back and forth between the two until they dont want to play anymore.
	debug				= A function used to test/debug code.
	checkValid		= Checks to see if the number is in the valid range of rMin and rMax.
	newGame			= Asks the user if they want to reset the scores.
 */
int randomNumber();
int smartGuess();
...
int randomNumber(int min, int max)
{
	...
	// Keeps generating a number until it is between the min and max
	do {
		randN = (rand() % RANDOM_MAX) + RANDOM_MIN;
...
{
	do {
		cout << "Please enter your number (" << RANDOM_MIN + 1<< "-" << RANDOM_MAX -1 << ")" << endl;
		cout << PROMPT_USR;
		cin >> pNum;
		cout << endl;
